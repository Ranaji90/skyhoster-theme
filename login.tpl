...
<form method="post" action="{routePath('login-validate')}" class="login-form" role="form">
    <div class="card mw-540 mb-md-4 mt-md-4">
        <div class="card-body px-sm-5 py-4">
            ...
            {/if}
            <div class="form-group">
                <button id="login" style="width:100%;" type="submit" class=" login-button btn btn-primary{$captcha->getButtonClass($captchaForm)}">
           ...
           {* <div class="text-right">
            ...
            <div class="text-center">
                        <a href="{routePath('password-reset-begin')}" class="small text-muted forgot-password" tabindex="-1">{lang key='forgotpw'}</a>
                    </div>  
        </div>
        <div class="card-footer card-footer-1 px-md-5 text-center">
           {* <small>{lang key='userLogin.notRegistered'}</small> *}
            <a href="{$WEB_ROOT}/register.php" class="small font-weight-bold createnaccount">{lang key='userLogin.createAccount'}</a>
        </div>
    </div>
</form>
